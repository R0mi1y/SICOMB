...
def confirm_load(request):
    ...
    if request.method == "POST":
        ...
        load = Load(expected_load_return_date=data_hora_futura, turn_type=turn_type, police=police)  # Cadastra a carga
        load.save()
...
def get_loads_police(request, plate):
    ...
    for load in loads_filtrados:
        ...
        dicionario_load['itens_amount'] = Equipment_load.objects.filter(load=load).__len__()
        dicionario_load['date_load'] = convert_date(str(dicionario_load['date_load']))
        if dicionario_load['expected_load_return_date']:
            dicionario_load['expected_load_return_date'] = convert_date(str(dicionario_load['expected_load_return_date']))
...
# @login_required
def get_load(request, id):  # Retorna uma resposta JSON com todas as cargas (caso necessário)
    ...
    for load in Equipment_load.objects.filter(load=load):
        ...
        if load.equipment.armament != None:  # Recupera o objeto armamento, que complementa o equipamento
            equipment["registred"] = "armament"
            equipment["model"] = model_to_dict(load.equipment.armament)

        elif load.equipment.wearable != None:  # Recupera o objeto vestimento, que complementa o equipamento
            equipment["registred"] = "wearable"
            equipment["model"] = model_to_dict(load.equipment.wearable)

        elif load.equipment.accessory != None:  # Recupera o objeto acessorio, que complementa o equipamento
            equipment["registred"] = "accessory"
            equipment["model"] = model_to_dict(load.equipment.accessory)

        elif load.equipment.grenada != None:  # Recupera o objeto acessorio, que complementa o equipamento
...
def get_dashboard_cargas(request):
    ...
    return render(
        request, "load/dashboard-load.html", {"loads": loads_aux, "": qnt}
    )
